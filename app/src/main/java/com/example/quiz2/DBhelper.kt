...
class DBhelper(val context: Context, val factory: SQLiteDatabase.CursorFactory?) :
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val users = "CREATE TABLE users (id INT PRIMARY KEY, login TEXT, email TEXT, password TEXT)"
        db!!.execSQL(users)
        val adminInsert = "INSERT INTO users VALUES('', 'admin', 'the', 'best')"
        db.execSQL(adminInsert)
        val quest = ("CREATE TABLE " + TABLE_NAME + " ("
                + NAME_COl + " TEXT, "
                + QUESTION_COL + " TEXT, "
                + ANSWER_COL + " TEXT, "
    ...
    fun getUser(login: String, password: String): Boolean {
        ...
        val result = db.rawQuery("SELECT * FROM users WHERE (login = '$login' OR email = '$login') AND password = '$password'", null)

        return result.moveToFirst()

    }

    fun isAdmin (login: String, password: String): Boolean{
        val db = this.readableDatabase

        val result = db.rawQuery("SELECT * FROM users WHERE (login = '$login' AND '$login' = 'admin') AND (password = '$password' AND '$password' = 'best')", null)
